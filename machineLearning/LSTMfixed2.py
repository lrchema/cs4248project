...
def train_model(model, X_train, y_train, X_test, y_test):
    ...
    history = model.fit(X_train, y_train, epochs=epochs, batch_size=batch_size, validation_data=(X_test, y_test),sample_weight=sample_weight,callbacks=[EarlyStopping(monitor='val_loss', patience=5, min_delta=0.0001)])
    return history
...
#    print('Test set\n  Loss: {:0.3f}\n  Accuracy: {:0.3f}'.format(accr[0],accr[1]))
    ...
    # Save this model
    pickleFile = f'trainedLSTM-{unique_id}.sav'
    ...
    plt.savefig(f"LSTMTraining-{unique_id}.jpg")
